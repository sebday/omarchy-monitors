...
Item {
  ...
  function beginBarMove(window) {
    ...
    barMoveCandidate = positionForScreen(barMoveScreen)
    barMoveActive = true
  }
  ...
  function finishBarMove() {
    ...
    var screen = barMoveScreen
    if (!barMoveActive || !edge || edge === positionForScreen(screen)) {
      clearBarMove()
      return
    }

    clearBarMove()
    setBarPosition(edge, screen)
  }

  function setBarPosition(value, screen) {
    var next = normalizePosition(value)
    var output = screen && screen.name ? String(screen.name) : ""
  ...
  component BarPanel: PanelWindow {
    ...
    readonly property string edge: root.positionForScreen(barWindow.screen)
    ...
    margins {
      top: root.barHidden && edge === "top" ? -edgeBarSize : 0
      bottom: root.barHidden && edge === "bottom" ? -edgeBarSize : 0
      left: root.barHidden && edge === "left" ? -edgeBarSize : 0
      right: root.barHidden && edge === "right" ? -edgeBarSize : 0
    }
    ...
    implicitWidth: edgeVertical ? edgeBarSize : 0
    implicitHeight: edgeVertical ? 0 : edgeBarSize
    ...
    Loader {
      anchors.fill: parent
      sourceComponent: edgeVertical ? verticalBar : horizontalBar
